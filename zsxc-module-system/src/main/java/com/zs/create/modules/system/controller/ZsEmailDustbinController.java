...
@Slf4j
public class ZsEmailDustbinController {
     ...
     * 分页列表查询
     */
    @AutoLog(value = "垃圾箱-分页列表查询")
    @ApiOperation(value = "垃圾箱-分页列表查询", notes = "垃圾箱-分页列表查询")
    ...
    public Result<IPage<ZsEmailVO>> queryPageList(ZsEmailVO zsEmailVO,
        ...
        IPage<ZsEmailVO> pageList = zsEmailService.searchDustbin(page, zsEmailVO, keyword);
     ...
     * 通过id删除
     */
    @AutoLog(value = "垃圾箱-通过id删除")
    @ApiOperation(value = "垃圾箱-通过id删除", notes = "垃圾箱-通过id删除")
    @DeleteMapping(value = "/delete")
    public Result<?> delete(@RequestParam(name = "id", required = true) String id) {
        try {
            zsEmailRecordingService.delte(id);
        } catch (Exception e) {
    ...
    @AutoLog(value = "垃圾箱-批量删除")
    @ApiOperation(value = "垃圾箱-批量删除", notes = "垃圾箱-批量删除")
    ...
    public Result<ZsEmailVO> deleteBatch(@RequestParam(name = "ids", required = true) String ids) {
        ...
        if (ids == null || "".equals(ids.trim())) {
            ...
            for (int i = 0; i < list.size(); i++) {
                zsEmailRecordingService.delte(list.get(i));
            }
            result.success("删除成功!");
        }
        return result;
    ...
    @AutoLog(value = "垃圾箱-通过id查询")
    @ApiOperation(value = "垃圾箱-通过id查询", notes = "垃圾箱-通过id查询")
    ...
    @AutoLog(value = "垃圾箱-恢复")
    @ApiOperation(value = "垃圾箱-恢复", notes = "垃圾箱-恢复")
    @DeleteMapping(value = "/resume")
    public Result<?> resume(@RequestParam(name = "id", required = true) String id) {
        try {
            ZsEmailRecording zsEmailRecord = new ZsEmailRecording();
            zsEmailRecord.setEmailId(id);
            zsEmailRecord.setDelFlag(0);
            zsEmailRecordingService.updateStatus(zsEmailRecord);
        } catch (Exception e) {
